            ...
            <div class="form-row">
              <div class="form-group">
                ...
                @if (f['username'].invalid && f['username'].touched) {
                  <span class="error-msg">Username required (3-20 chars)</span>
                }
              </div>
              <div class="form-group">
...
export class RegisterComponent {
  ...
  requestOtp(): void {
    if (this.f['fullName'].invalid || this.f['username'].invalid || this.f['email'].invalid || this.f['password'].invalid || this.f['confirmPassword'].invalid || this.form.errors?.['mismatch']) {
    ...
    this.auth.sendOtp({ email: this.f['email'].value, type: 'VERIFICATION' }).subscribe({
      next: () => {
